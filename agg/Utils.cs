...
namespace CalendarAggregator
{
    ...
    public static class Utils
    {
        ...
        {
            var values = Utils.RegexFindAll(datekey, EventStore.datekey_pattern);
        ...
        public static string[] FindCityOrTownAndStateAbbrev(string where)
        {
            var city_or_town = "";
            var state_abbrev = "";
            var groups = RegexFindAll(where, @"(.+)([\s+,])([^\s]+)");
            if (groups.Count > 1)
            {
        ...
        public static string[] LookupLatLon(string appid, string where)
        {
            ...
            if (resp.status == HttpStatusCode.OK)
            {
                ...
                lat = Utils.RegexFindAll(xml, "<Latitude>([^<]+)")[1];
                lon = Utils.RegexFindAll(xml, "<Longitude>([^<]+)")[1];
            }
        ...
        public static string iCal4J_Validate(string str_url)
        {
            ...
            {
                error = Utils.RegexFindAll(page, "Error was: ([^<]+)")[1];
            }
            return error;
        }
